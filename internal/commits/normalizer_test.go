...
func TestNormalize(t *testing.T) {
	tests := []struct {
		...
		{
			name: "preserves breaking-change marker",
			in:   "feat!: drop support for v1",
			want: "feat!: drop support for v1",
		},
		{
			name: "preserves breaking-change marker with scope",
			in:   "feat(api)!: change response shape",
			want: "feat(api)!: change response shape",
		},
		{
			name: "lowercases type with breaking-change marker",
			in:   "FIX!: critical security hole",
			want: "fix!: critical security hole",
		},
		{
			name: "accepts revert type",
			in:   "revert: bring back old login",
			want: "revert: bring back old login",
		},
		{
		...
		{
			name: "wraps body by runes for multibyte text",
			in:   "docs: add glossary\n\n" + strings.Repeat("é", 30) + " " + strings.Repeat("é", 30) + " " + strings.Repeat("é", 30),
			want: "docs: add glossary\n\n" +
				strings.Repeat("é", 30) + " " + strings.Repeat("é", 30) + "\n" + strings.Repeat("é", 30),
		},
		{
