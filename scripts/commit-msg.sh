...
# Capture stderr to a private temp file so the diagnostics below can replay it.
ERR_FILE="$(mktemp -t nightshift-commit-msg.XXXXXX)"
...
# Run normalize, tolerating its non-zero exit (a failing command substitution
# would otherwise abort the script under `set -e` before the diagnostics run).
STATUS=0
NORMALIZED="$("$NIGHTSHIFT" commit normalize --file "$MSG_FILE" 2>"$ERR_FILE")" || STATUS=$?
