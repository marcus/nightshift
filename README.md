...
This installs symlinks into Git's active hooks directory, so it works with the default `.git/hooks`, `core.hooksPath`, and linked worktrees. `make install-hooks` installs both `pre-commit` and `commit-msg`.
